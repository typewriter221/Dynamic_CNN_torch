...
class DynamicConvolution(TempModule):
    ...
    def forward(self, x, temperature=1):
        ...
        # print("Atttention Size",alphas.shape, "No. of Kernels", self.nof_kernels)
        # print("Kerrnel Weights: ",self.kernels_weights.unsqueeze(0).shape)
        # print("Alphas: ", alphas.view(batch_size, -1, 1, 1, 1, 1).shape)
        ...
        # print("X shape: ",x_grouped.shape, "Agg weight: ", agg_weights.shape)
...
class AttentionLayer3d(nn.Module):
    def __init__(self, c_dim, hidden_dim, nof_kernels):
        super().__init__()
        self.global_pooling = nn.Sequential(nn.AdaptiveAvgPool3d(1), nn.Flatten())
        self.to_scores = nn.Sequential(nn.Linear(c_dim, hidden_dim),
                                       nn.ReLU(inplace=True),
                                       nn.Linear(hidden_dim, nof_kernels))
...
class DynamicConvolution3d(TempModule):
    def __init__(self, nof_kernels, reduce, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, dilation=1, groups=1, bias=True):
        """
        Implementation of Dynamic convolution layer
        ...
        self.conv_args = {'stride': stride, 'padding': padding, 'dilation': dilation}
        self.nof_kernels = nof_kernels
        self.attention = AttentionLayer3d(in_channels, max(1, in_channels // reduce), nof_kernels)
    ...
    def forward(self, x, temperature=1):
        ...
        # print("Atttention Size",alphas.shape, "No. of Kernels", self.nof_kernels)
        # print("Kerrnel Weights: ",self.kernels_weights.unsqueeze(0).shape)
        # print("Alphas: ", alphas.view(batch_size, -1, 1, 1, 1, 1, 1).shape)
        agg_weights = torch.sum(
            torch.mul(self.kernels_weights.unsqueeze(0), alphas.view(batch_size, -1, 1, 1, 1, 1, 1)), dim=1)
        # Group the weights for each batch to conv3 all at once
        # print("Agg weights Calc: ", agg_weights.shape)
        agg_weights = agg_weights.view(-1, *agg_weights.shape[-4:])  # batch_size*out_c X in_c X kernel_size X kernel_size X kernel_size
        # print("Agg weights Resized: ", agg_weights.shape)
        if self.kernels_bias is not None:
            agg_bias = torch.sum(torch.mul(self.kernels_bias.unsqueeze(0), alphas.view(batch_size, -1, 1)), dim=1)
            agg_bias = agg_bias.view(-1)
        else:
            agg_bias = None
        x_grouped = x.view(1, -1, *x.shape[-3:])  # 1 X batch_size*out_c X depth X H X W
        # print(x_grouped.shape)
        # print(self.conv_args) 
        # print("X shape: ",x_grouped.shape, "Agg weight: ", agg_weights.shape)  
        out = F.conv3d(x_grouped, agg_weights, agg_bias, groups=self.groups * batch_size,
                       **self.conv_args)  # 1 X batch_size*out_C X H' x W'
        out = out.view(batch_size, -1, *out.shape[-3:])  # batch_size X out_C X H' x W'
...
if __name__ == '__main__':
    ...
    # conv = DynamicConvolution(3, 1, in_channels=3, out_channels=8, kernel_size=3, padding=1,stride=1, bias=True)
    t = torch.randn(87, 1, 4, 200, 200)
    conv = DynamicConvolution3d(3, 1, in_channels=1, out_channels=8, kernel_size=3, padding=1,stride=1, bias=True)
    # print(conv(t,10))
    print(conv(t, 10).shape)
